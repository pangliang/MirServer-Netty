...
public class NumUtil {
	...
	public static int randomRang(int min, int max) {
		return min + (max - min > 0 ? nextRandomInt(max - min) : 0);
	}
	 ...
	 *
	 * @param bytes   数据来源
	 * @param index   数组中获取数据的起始位置(索引从0开始，数据还原将包含给定位置) 具体规则示例参见{@link #readShort(byte[], int, boolean) readShort}
	 * @param reverse 是否需要反转字节(只针对包含当前数据的字节数组)
	 *
	 * @return 从字节数组指定位置4个字节还原出的短整形数值
	 */
	 ...
	 *
	 * @param bytes   数据来源
	 * @param index   数组中获取数据的起始位置(索引从0开始，数据还原将包含给定位置)
	 *                <br>
	 *                例如给定[0x01,0x02,0x03,0x04],2,true作为参数则函数会将[0x04,0x03]还原为短整形数值并返回
	 * @param reverse 是否需要反转字节(只针对包含当前数据的字节数组)
	 *
	 * @return 从字节数组指定位置2个字节还原出的短整形数值
	 */
