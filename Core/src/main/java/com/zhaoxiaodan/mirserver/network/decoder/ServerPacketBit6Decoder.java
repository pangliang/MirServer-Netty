...
public class ServerPacketBit6Decoder extends MessageToMessageDecoder<ByteBuf> {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		...
		if ('+' == in.getByte(1)) {
			...
			byte[] headerByte = new byte[ServerPacket.DEFAULT_HEADER_SIZE * 4 / 3];
			in.readBytes(headerByte);
			buf.writeBytes(Bit6Coder.decode6BitBuf(headerByte));

			if (buf.order(ByteOrder.LITTLE_ENDIAN).getShort(4) == Protocol.SM_TURN.id) {
				if (in.readableBytes() > 4 * 2 * 4 / 3 + 1) {
					byte[] bodyBytes = new byte[4 * 2 * 4 / 3 + 1];
